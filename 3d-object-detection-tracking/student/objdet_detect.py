...
def detect_objects(input_bev_maps, model, configs):
    ...
    if len(detections):
        ...
        for det in detections:
            ...
            w = w / configs.bev_width * (configs.lim_x[1] - configs.lim_x[0])
            l = l / configs.bev_height * (configs.lim_y[1] - configs.lim_y[0])
            
            ## step 4 : append the current object to the 'objects' array
            x_inside = (x >= configs.lim_x[0]) and (x <= configs.lim_x[1])
            y_inside = (y >= configs.lim_y[0]) and (y <= configs.lim_y[1])
            z_inside = (z >= configs.lim_z[0]) and (z <= configs.lim_z[1])
        
            ## step 4 : append the current object to the 'objects' array
            if x_inside and y_inside and z_inside:
                objects.append([1, x, y, 0, 1.50, w, l, -yaw])
